...
import { PrismaClient, AcquisitionMethod, PlayerTagType, DraftColor, TransactionType } from "@prisma/client";
import { getDraftColor } from "../src/lib/draft-color-engine";
import { recomputeAllKeeperRecords } from "../src/lib/keeper-sync";
import {
  LEAGUE_NAME,
  LEAGUE_ABBREVIATION,
  KEEPER_SLOT_COUNT,
...
async function main() {
  ...
  const managerByTeamName = new Map<string, string>();
  for (const def of managerDefs) {
    ...
    managerByTeamName.set(def.team, manager.id);
  }
